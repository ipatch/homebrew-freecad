class NumpyAT1212 < Formula
  ...
  def install
    ...
    ENV.prepend_create_path "PYTHONPATH", Formula["./cython@0.29.24"].opt_libexec/"lib/python#{version}/site-packages:#{py.site_packages}"
  ...
  test do
    system Formula["./python@3.9.7"].opt_bin/"python3", "-c", <<~EOS
      import numpy as np
      t = np.ones((3,3), int)
      assert t.sum() == 9
